...
namespace Hasslefree.Business.Controllers.Rentals
{
    public class CompleteRentalLandlordSignatureController : BaseController
    {
        ...
        [HttpGet, Route("account/rental/l/complete-partner-signature")]
        public ActionResult CompletePartnerSignature(string hash)
        {
            if (SessionManager.IsLoggedIn())
            {
                LogoutService.Logout();
                return Redirect($"/account/rental/l/complete-partner-signature?hash={hash}");
            }

            var decodedHash = Encoding.UTF8.GetString(Convert.FromBase64String(hash));

            var rentalId = Int32.Parse(decodedHash.Split(';')[0]);
            var partnerNumber = Int32.Parse(decodedHash.Split(';')[1]);

            var rental = GetRental[rentalId].Get();
            if (partnerNumber == 1 && rental.RentalFica.Partner1SignatureId.HasValue) return Redirect("/account/rental/l/complete-partner-signature/success");
            if (partnerNumber == 2 && rental.RentalFica.Partner2SignatureId.HasValue) return Redirect("/account/rental/l/complete-partner-signature/success");
            if (partnerNumber == 3 && rental.RentalFica.Partner3SignatureId.HasValue) return Redirect("/account/rental/l/complete-partner-signature/success");

            var model = new CompleteRentalPartnerSignature
            {
                PartnerNumber = partnerNumber,
                RentalId = rental.RentalId
            };

            ViewBag.Title = "Complete Partner Signature";

            // Ajax
            if (WebHelper.IsAjaxRequest()) return PartialView("../Rentals/CompletePartnerSignature", model);

            // Default
            return View("../Rentals/CompletePartnerSignature", model);
        }

        [HttpPost, Route("account/rental/l/complete-partner-signature")]
        public ActionResult CompletePartnerSignature(CompleteRentalPartnerSignature model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    if (model.PartnerNumber == 1)
                    {
                        name = rental.RentalFica.Partner1Name;
                        surname = rental.RentalFica.Partner1Surname;
                    }

                    if (model.PartnerNumber == 2)
                    {
                        name = rental.RentalFica.Partner2Name;
                        surname = rental.RentalFica.Partner2Surname;
                    }

                    if (model.PartnerNumber == 3)
                    {
                        name = rental.RentalFica.Partner3Name;
                        surname = rental.RentalFica.Partner3Surname;
                    }
                    ...
                    if (model.PartnerNumber == 1)
                    {
                        success = UpdateRentalFicaService.WithRentalId(rental.RentalWitness.RentalId)[rental.RentalFica.RentalFicaId]
                        .Set(a => a.Partner1SignatureId, pictures.FirstOrDefault(p => p.Name == $"{name.ToLower().Replace(" ", "-")}_{surname.ToLower().Replace(" ", "-")}_signature.png").PictureId)
                        .Update();
                    }
                    ...
                    {
                        success = UpdateRentalFicaService.WithRentalId(rental.RentalWitness.RentalId)[rental.RentalFica.RentalFicaId]
                         .Set(a => a.Partner2SignatureId, pictures.FirstOrDefault(p => p.Name == $"{name.ToLower().Replace(" ", "-")}_{surname.ToLower().Replace(" ", "-")}_signature.png").PictureId)
                         .Update();
                    }
                    ...
                    {
                        success = UpdateRentalFicaService.WithRentalId(rental.RentalWitness.RentalId)[rental.RentalFica.RentalFicaId]
                         .Set(a => a.Partner3SignatureId, pictures.FirstOrDefault(p => p.Name == $"{name.ToLower().Replace(" ", "-")}_{surname.ToLower().Replace(" ", "-")}_signature.png").PictureId)
                         .Update();
                    }
                    ...
                    if (success)
                    {
                        ...
                        bool allSigned = true;
                        if (!String.IsNullOrEmpty(rental.RentalFica.Partner1Email) && !rental.RentalFica.Partner1SignatureId.HasValue) allSigned = false;
                        if (!String.IsNullOrEmpty(rental.RentalFica.Partner2Email) && !rental.RentalFica.Partner2SignatureId.HasValue) allSigned = false;
                        if (!String.IsNullOrEmpty(rental.RentalFica.Partner3Email) && !rental.RentalFica.Partner3SignatureId.HasValue) allSigned = false;
                        ...
                        // Default
                        return Redirect($"/account/rental/l/complete-partner-signature/success");
                    }
                }
            }
            catch (Exception ex)
        ...
        [HttpGet, Route("account/rental/l/complete-partner-signature/success")]
        public ActionResult CompletePartnerSignatureSuccess()
        {
            if (SessionManager.IsLoggedIn())
            {
                LogoutService.Logout();
            ...
            ViewBag.Title = "Completed Partner Signature";

            // Ajax
            if (WebHelper.IsAjaxRequest()) return PartialView("../Rentals/CompletePartnerSignatureSuccess");

            // Default
            return View("../Rentals/CompletePartnerSignatureSuccess");
        }
        ...
        public ActionResult CompleteLandlordSignature(CompleteRentalLandlordSignature model)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    {
                        if (!String.IsNullOrEmpty(rental.RentalFica?.Partner1Email)) SendPartnerSignatureEmail(rental.RentalFica.Partner1Email, rental.RentalId, 1);
                        if (!String.IsNullOrEmpty(rental.RentalFica?.Partner2Email)) SendPartnerSignatureEmail(rental.RentalFica.Partner2Email, rental.RentalId, 2);
                        if (!String.IsNullOrEmpty(rental.RentalFica?.Partner3Email)) SendPartnerSignatureEmail(rental.RentalFica.Partner3Email, rental.RentalId, 3);

                        success = UpdateRentalService[rental.RentalId]
                        .Set(a => a.RentalStatus, RentalStatus.PendingPartnerSignatures)
                        .Update();
                    }
        ...
        private bool SendPartnerSignatureEmail(string email, int rentalId, int partnerNumber)
        {
            var url = $"account/rental/emails/partner-signature-email?rentalId={rentalId}&partnerNumber={partnerNumber}";

            SendMail.WithUrlBody(url).WithRecipient(email);

            return SendMail.Send("New Listing - Partner Signature");
        }
