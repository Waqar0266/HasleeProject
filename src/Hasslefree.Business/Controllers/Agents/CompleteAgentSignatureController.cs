		...
		public ActionResult CompleteSignature(CompleteAgentSignature model)
		{
			try
			{
				if (ModelState.IsValid)
				{
					...
					.WithField("Title", $"{person.FirstName.ToUpper()} {person.Surname.ToUpper()} ({agent.IdNumber})")
					...
					.WithField("SignedAt", residential.Town)
					...
					.WithField("Id_1", $"{agent.IdNumber[0]}")
					.WithField("Id_2", $"{agent.IdNumber[1]}")
					.WithField("Id_3", $"{agent.IdNumber[2]}")
					.WithField("Id_4", $"{agent.IdNumber[3]}")
					.WithField("Id_5", $"{agent.IdNumber[4]}")
					.WithField("Id_6", $"{agent.IdNumber[5]}")
					.WithField("Id_7", $"{agent.IdNumber[6]}")
					.WithField("Id_8", $"{agent.IdNumber[7]}")
					.WithField("Id_9", $"{agent.IdNumber[8]}")
					.WithField("Id_10", $"{agent.IdNumber[9]}")
					.WithField("Id_11", $"{agent.IdNumber[10]}")
					.WithField("Id_12", $"{agent.IdNumber[11]}")
					.WithField("Id_13", $"{agent.IdNumber[12]}")
					.WithField("Date of Birth", CalculateDateOfBirth(agent.IdNumber).ToString("yyyy/MM/dd"))
					...
					.WithField("AgentNameSurname", $"{person.FirstName} {person.Surname}")
					.WithField("IdNumber", agent.IdNumber)
					.WithField("SignedAt", residential.Town)
					.WithField("SignedDay", DateTime.Now.ToString("dd"))
					.WithField("SignedMonthYear", DateTime.Now.ToString("MMMM") + " " + DateTime.Now.ToString("yyyy"))
					.Process();
					...
					UploadDownload.Add(new Web.Models.Media.Downloads.DownloadModel()
					{
						...
						DownloadType = Core.Domain.Media.DownloadType.Document,
						Extension = "pdf",
						FileName = $"{model.Name.ToLower().Replace(" ", "-")}_{model.Surname.ToLower().Replace(" ", "-")}_agent_contract_{DateTime.Now.ToString("yyyyMMddHHmm")}.pdf",
						Key = $"{agent.AgentGuid}/{model.Name.ToLower().Replace(" ", "-")}_{model.Surname.ToLower().Replace(" ", "-")}_agent_contract_{DateTime.Now.ToString("yyyyMMddHHmm")}.pdf",
						MediaStorage = Core.Domain.Media.MediaStorage.Cloud,
					...
					success = CreateAgentForm.New(FormName.Eaab, agent.AgentId, downloads.FirstOrDefault(d => d.FileName == $"{model.Name.ToLower().Replace(" ", "-")}_{model.Surname.ToLower().Replace(" ", "-")}_eaab_{DateTime.Now.ToString("yyyyMMddHHmm")}.pdf").DownloadId).Create();
					success = CreateAgentForm.New(FormName.AgentContract, agent.AgentId, downloads.FirstOrDefault(d => d.FileName == $"{model.Name.ToLower().Replace(" ", "-")}_{model.Surname.ToLower().Replace(" ", "-")}_agent_contract_{DateTime.Now.ToString("yyyyMMddHHmm")}.pdf").DownloadId).Create();
