...
namespace Hasslefree.Business.Controllers.Rentals
{
	public class CompleteRentalAgentSignatureController : BaseController
	{
		...
		[HttpGet, Route("account/rental/a/complete-partner-signature")]
		public ActionResult CompletePartnerSignature(string hash)
		{
			if (SessionManager.IsLoggedIn())
			{
				LogoutService.Logout();
				return Redirect($"/account/rental/a/complete-partner-signature?hash={hash}");
			}
			...
			if (partnerNumber == 1 && rental.RentalFica.Partner1SignatureId.HasValue) return Redirect("/account/rental/a/complete-partner-signature/success");
			if (partnerNumber == 2 && rental.RentalFica.Partner2SignatureId.HasValue) return Redirect("/account/rental/a/complete-partner-signature/success");
			if (partnerNumber == 3 && rental.RentalFica.Partner3SignatureId.HasValue) return Redirect("/account/rental/a/complete-partner-signature/success");
		...
		private void FillForms(RentalGet rental)
		{
			...
			switch (rental.LeaseType)
			{
				...
				case LeaseType.ClosedCorporation:
					{
						mandateAgreementName = "Mandate Agreement - Closed Corporation.pdf";
						powerOfAttorneyName = "Special Power of Attorney – Juristic.pdf";

						//fill fica prescribed form
						var ficaPrescribedFormData = FillForm.Prepare("FICA PRESCRIBED CLIENT FORM (JURISTIC PERSON).pdf")
						...
						.WithField("Member1DateOfBirth", CalculateDateOfBirth(rental.RentalFica.Partner1IdNumber).ToString("yyyy-MM-dd"))
						...
						.WithField("Member2DateOfBirth", CalculateDateOfBirth(rental.RentalFica.Partner2IdNumber).ToString("yyyy-MM-dd"))
						...
						.WithField("Member3DateOfBirth", CalculateDateOfBirth(rental.RentalFica.Partner3IdNumber).ToString("yyyy-MM-dd"))
			...
			if (rental.PowerOfAttorney)
			{
				...
				UploadDownload.Add(new Web.Models.Media.Downloads.DownloadModel()
				{
					...
					MediaStorage = Core.Domain.Media.MediaStorage.Cloud,
					Data = specialPowerofAttroney,
					Size = specialPowerofAttroney.Length
				});
