...
{
	public class EftReference  : IEmailDefinition
	{
		public string Title => "EFT Reference";
		...
		public Object Example =>
			new
			{
				...
				Document = new OrderEmailModel()
				{
					...
					Banks = new List<OrderBankEmailModel>()
					{
						new OrderBankEmailModel()
						{
							Name = "Bank 1",
							BankName = "Name",
		...
		public InstallDefinition Install => new InstallDefinition()
		{
			Send = true,
			From = "",
			Subject = "EFT Reference - {0}",
			Recipient = "",
			Url = "/order/eft-email/{0}"
		};
	}
}
